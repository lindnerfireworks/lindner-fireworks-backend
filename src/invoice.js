// Erzeugt die PDF-Rechnung zu einer Bestellung direkt im Server – kein
...
/**
 * Erzeugt die Rechnung als PDF (Buffer) für eine gespeicherte Bestellung.
 * Erwartet: { invoiceNumber, invoiceDate, customerName, items, total, abholtermin }
...
function generateInvoicePdf(order) {
  return new Promise((resolve, reject) => {
    try {
      ...
      const invoiceDate = order.invoiceDate || new Date().toLocaleDateString("de-AT");
      ...
      try {
        ...
        // Falls das Logo mal nicht gefunden wird, Rechnung trotzdem ohne Logo erzeugen.
      }
      doc.fontSize(22).fillColor(PINK).font("Helvetica-Bold").text("RECHNUNG", 0, 58, { align: "right" });
      ...
      doc
        ...
        .fillColor(GRAY)
        .font("Helvetica-Bold")
        .text("RECHNUNGSEMPFÄNGER", 300, topY)
        .font("Helvetica")
        .fillColor(DARK)
        .fontSize(10)
      ...
      doc
        ...
        .fillColor(DARK)
        .font("Helvetica-Bold")
        .text("Rechnungsnummer: ", 50, metaY, { continued: true })
        .font("Helvetica")
        .text(invoiceNumber);
      doc.font("Helvetica-Bold").text("Rechnungsdatum: ", 300, metaY, { continued: true }).font("Helvetica").text(invoiceDate);
